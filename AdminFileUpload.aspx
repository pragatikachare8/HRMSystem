<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AdminFileUpload.aspx.cs" Inherits="HRMSAdminModule.AdminFileUpload" %>
                            ...
                            <div class="card-body">
                                ...
                                <div class="basic-form">
                                    <div class="form-control">
                                       <asp:ListBox ID="ListBox1" runat="server" SelectionMode="Multiple" CssClass="multi-select">
</asp:ListBox>
                                    </div>

                                    <div class="form-group">
                                        <asp:DropDownList ID="DropDownList1" runat="server" class="form-control input-rounded">
                                            <asp:ListItem Value="Select Document Type"></asp:ListItem>
                                            <asp:ListItem Value="Payslip"></asp:ListItem>
                                            <asp:ListItem Value="Offer Letter"></asp:ListItem>
                                            <asp:ListItem Value="Others"></asp:ListItem>
                                        </asp:DropDownList>
                                    </div>
                                    <div class="form-group">
                                        <asp:FileUpload ID="FileUpload1" runat="server" class="form-control input-rounded" />
                                    </div>
                                    <div class="form-group">
                                        <asp:Button ID="Button1" runat="server" class="btn btn-dark" Text="Upload" OnClick="Button1_Click" />
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    ***********************************-->
    ...
    $(document).ready(function () {
        $('#uploadBtn').on('click', function (e) {
            ...
            var docType = $('#DropDownList1').val();
            var fileInput = $('#FileUpload1')[0].files[0]; // Get the first file from the FileUpload control

            // Ensure file and document type are selected
            if (!fileInput || docType === "Select Document Type") {
                alert("Please select a document type and a file.");
                return;
            }

            // Prepare FormData object for AJAX
            var formData = new FormData();
            formData.append('DocName', docType);
            formData.append('File', fileInput);

            // Optionally, add the selected employee(s) from ListBox1
            ...
            $.ajax({
                url: 'DocumentUploadHandler.aspx', // This is the URL where your backend logic is written
                type: 'POST',
                data: formData,
                processData: false, // Don't process the files
                contentType: false, // Don't set content type header
                success: function (response) {
                    // Handle response from the server (e.g., success or error)
